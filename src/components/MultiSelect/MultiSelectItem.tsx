...
export function MultiSelectItem({ value, disabled = false, children, className }: MultiSelectItemProps) {
  ...
  useEffect(() => ctx.registerItem(value, { label: children, disabled }), [value, disabled, children]);   
  ...
  return (
    <div
      ...
      onClick={() => !disabled && ctx.toggleValue(value)}       className={cn(
        "flex cursor-pointer items-center justify-between rounded-[calc(var(--kui-radius)-2px)] px-2.5 py-1.5 text-sm text-text transition-colors",
