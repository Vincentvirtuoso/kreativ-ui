...
import { multiSelectSizeVariants, chipSizeVariants } from "./MultiSelect.styles";

function orderedEnabledValues(items: Map<string, { disabled?: boolean }>) {
  return [...items.entries()].filter(([, meta]) => !meta.disabled).map(([val]) => val);
}

export const MultiSelectTrigger = forwardRef<HTMLButtonElement, { className?: string }>(
  ({ className }, ref) => {
    ...
    const visible = ctx.maxVisibleChips ? ctx.value.slice(0, ctx.maxVisibleChips) : ctx.value;
      ...
      >
        ...
        >
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  onClick={(e) => { e.stopPropagation(); ctx.removeValue(v); }}
          ...
          {overflow > 0 && (
            <span className={cn(
              "text-text-muted",
              ctx.size === "sm" ? "text-[10px]" : ctx.size === "md" ? "text-xs" : "text-sm"
            )}>
              +{overflow} more
            </span>
          )}
