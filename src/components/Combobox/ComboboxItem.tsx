...
export function ComboboxItem({ value, disabled = false, children, className }: ComboboxItemProps) {
    const ctx = useComboboxContext("ComboboxItem");
    const ref = useRef<HTMLDivElement>(null);
    const active = ctx.activeValue === value;
    const selected = ctx.value === value;

    useEffect(() => ctx.registerItem(value, { label: children, disabled, visible: true }), [value, disabled, children]);
    ...
    const matches =
        !query ||
        String(children).toLowerCase().includes(query);
