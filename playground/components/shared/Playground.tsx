...
export function Playground({
    ...
    useEffect(() => {
        ...
        const el = wrapperRef.current?.querySelector("button") || wrapperRef.current?.querySelector("input");
        if (!el) return;
            ...
            <div className="grid grid-cols-[300px_1fr] gap-6 max-lg:grid-cols-1">
                ...
                <div>
                    ...
                    <div className="mt-5 rounded-xl border border-border bg-surface p-5">
                        <p className="mb-2 font-mono text-[11px] text-text-muted">generated jsx</p>
                        <pre className="overflow-x-auto rounded-[var(--kui-radius)] border border-border bg-bg p-3.5 font-mono text-xs leading-relaxed text-text-muted">
                            {code}
                        </pre>
                    </div>
                    ...
                    {getAttributes && (
                        <div className="mt-5 rounded-xl border border-border bg-surface p-5">
                            <p className="mb-2 font-mono text-[11px] text-text-muted">resolved dom attributes</p>
                            <div className="divide-y divide-border">
                                {Object.entries(domAttrs).map(([k, v]) => (
                                    <div key={k} className="flex justify-between gap-3 py-1.5 font-mono text-xs">
                                        <span className="text-text-muted">{k}</span>
                                        <span className={v ? "text-brand" : "text-text-muted"}>{v ?? "—"}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
