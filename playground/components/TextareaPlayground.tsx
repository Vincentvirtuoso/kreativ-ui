...
export function TextareaPlayground() {
    ...
    const controls = (
        <>
            ...
            <SegmentedControl label="resize" value={resize} options={RESIZE_OPTIONS} onChange={setResize} />

            <div className="mb-4 space-y-2">
                <div>
                    <label className="block text-xs font-mono text-text-muted mb-1">placeholder</label>
                    <input
                        type="text"
                        value={placeholder}
                ...
                <div className="flex gap-2">
                    <div className="flex-1">
                        <label className="block text-xs font-mono text-text-muted mb-1">minRows</label>
                        <input
                            type="number"
                            value={minRows ?? ""}
                    ...
                    <div className="flex-1">
                        <label className="block text-xs font-mono text-text-muted mb-1">maxRows</label>
                        <input
                            type="number"
                            value={maxRows ?? ""}
                ...
                <div>
                    <label className="block text-xs font-mono text-text-muted mb-1">maxLength</label>
                    <input
                        type="number"
                        value={maxLength ?? ""}
            ...
            <div className="flex flex-wrap gap-1.5">
                ...
                <Chip active={autoResize} onClick={() => setAutoResize((v) => !v)}>autoResize</Chip>
                <Chip active={clearable} onClick={() => setClearable((v) => !v)}>clearable</Chip>
                <Chip active={characterCounter} onClick={() => setCharacterCounter((v) => !v)}>characterCounter</Chip>
                <Chip active={trimOnBlur} onClick={() => setTrimOnBlur((v) => !v)}>trimOnBlur</Chip>
                ...
                <Chip active={enableValidation} onClick={() => setEnableValidation((v) => !v)}>validation</Chip>
